...
This program performs testing on all maps in ../Maps/Testing_maps
'''
import time
import numpy as np
...
for id_N, N_agents in enumerate(N_agents_list):
    ...
    for id_map in range(int(args['N_testing_maps'])):
        # Reset the environment for testing
        map_path = "../Maps/Testing_Maps/Testing_map_"+str(id_map)
        states, positions, cells, trajectories, statistics = env.reset_test(args, random_choice= True, path = map_path)
        ind_increases = []
        cov_percs = []
